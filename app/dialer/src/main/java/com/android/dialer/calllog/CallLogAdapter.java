...
public class CallLogAdapter extends GroupingListAdapter
    ...
    private final SparseArray<Long> selectedNumIds = new SparseArray<>();
        ...
        public void onClick(View v) {
            ...
            if (selectMode) {
                if (viewHolder.isSelected) {
                    ...
                    selectedNumIds.delete(viewHolder.position);
                } else {
                    viewHolder.setSelected(true);
                    selectedNumIds.put(viewHolder.position, viewHolder.rowId);
                }
                if (mActivityType == ACTIVITY_TYPE_CALL_LOG) {
                    ((CallLogActivity) v.getContext()).onSelectAll(selectedNumIds.size() == getItemCount());
                    ((CallLogActivity) v.getContext()).onSelectSize(selectedNumIds.size());
                }
                return;
            }
            viewHolder.startDialActivity();
        }
    };
    ...
    private void bindCallLogListViewHolder(ViewHolder viewHolder, int position) {
        ...
        if (selectMode) {
            ...
            views.selectCheckBox.setVisibility(View.VISIBLE);
        } else {
            views.quickContactView.setVisibility(View.VISIBLE);
            views.primaryActionButtonView.setVisibility(View.VISIBLE);
            views.selectCheckBox.setVisibility(View.GONE);
        }
        views.updatePhoto();
    ...
    public void selectAll(boolean isInSelectAll) {
        if (isInSelectAll) {
            for (int i = 0; i< getItemCount(); i++) {
                ...
                selectedNumIds.put(i, c.getLong(CallLogQuery.ID));
            }
        } else {
            getSelectCallLogIds().clear();
        }
        notifyDataSetChanged();
    }

    public SparseArray<Long> getSelectCallLogIds() {
        return selectedNumIds;
    }

    private void updateCheckMarkedStatusOfEntry(CallLogListItemViewHolder views) {
        if (selectedNumIds.size() > 0 && selectedNumIds.get(views.position) != null) {
            views.selectCheckBox.setChecked(true);
        } else {
            views.selectCheckBox.setChecked(false);
        }
    }
