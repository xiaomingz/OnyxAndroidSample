...
fun Context.getFormattedTime(passedSeconds: Int, showSeconds: Boolean, makeAmPmSmaller: Boolean): SpannableString {
    ...
    return if (!use24HourFormat) {
        ...
        val appendable = get12HourFormatDesc(hours)
        val spannableTime = SpannableString(formattedTime)
        val amPmMultiplier = if (makeAmPmSmaller) 0.4f else 1f
        spannableTime.setSpan(RelativeSizeSpan(amPmMultiplier), spannableTime.length - appendable.length, spannableTime.length, 0)
        spannableTime
    } else {
...
fun Context.get12HourFormatDesc(hours: Int): String {
    return getString(if (hours >= 12) R.string.p_m else R.string.a_m)
}
