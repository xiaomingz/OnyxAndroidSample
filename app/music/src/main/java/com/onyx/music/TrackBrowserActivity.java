        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
    ...
    private TouchInterceptor.DropListener mDropListener =
        ...
        public void drop(int from, int to) {
            if (mTrackCursor instanceof NowPlayingCursor) {
                ...
                Playlists.Members.moveItem(getContentResolver(),
    ...
    private void removePlaylistItem(int which) {
        ...
        if (mTrackCursor instanceof NowPlayingCursor) {
            ...
            Uri uri = Playlists.Members.getContentUri("external",
    ...
    private void moveItem(boolean up) {
        ...
        if (mTrackCursor instanceof NowPlayingCursor) {
            ...
            Uri baseUri = Playlists.Members.getContentUri("external",
                    Long.valueOf(mPlaylist));
            ContentValues values = new ContentValues();
            String where = Playlists.Members._ID + "=?";
            String [] wherearg = new String[1];
            ContentResolver res = getContentResolver();
            if (up) {
                values.put(Playlists.Members.PLAY_ORDER, currentplayidx - 1);
            ...
            values.put(Playlists.Members.PLAY_ORDER, currentplayidx);
            wherearg[0] = mTrackCursor.getString(0);
            res.update(baseUri, values, where, wherearg);
        }
    }
