...
package com.onyx.deskclock.deskclock.data;
...
import com.onyx.deskclock.deskclock.timer.ExpiredTimersActivity;
import com.onyx.deskclock.deskclock.timer.TimerKlaxon;
import com.onyx.deskclock.deskclock.timer.TimerService;
...
//    private final Set<Integer> mRingingIds = new ArraySet<>();
    private final Set<Integer> mRingingIds = new HashSet<>();
    ...
    Timer addTimer(long length, String label, boolean deleteAfterUse) {
        ...
        Timer timer = new Timer(-1, Timer.State.RESET, length, length, Long.MIN_VALUE, length, label,
                deleteAfterUse);
    ...
    private void updateRinger(Timer before, Timer after) {
        ...
        if (afterState == Timer.State.EXPIRED && mRingingIds.add(after.getId()) && mRingingIds.size() == 1) {
        ...
        if (beforeState == Timer.State.EXPIRED && mRingingIds.remove(before.getId()) && mRingingIds.isEmpty()) {
    ...
    private void schedulePendingIntent(long triggerTime, PendingIntent pi) {
        if (Utils.isMOrLater()) {
            ...
            if (Build.VERSION.SDK_INT >= 19){
                ...
                mAlarmManager.set(ELAPSED_REALTIME_WAKEUP,triggerTime,pi);
            }
        }
    }
