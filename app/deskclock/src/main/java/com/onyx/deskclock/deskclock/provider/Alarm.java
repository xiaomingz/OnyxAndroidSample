...
package com.onyx.deskclock.deskclock.provider;
...
public final class Alarm implements Parcelable, ClockContract.AlarmsColumns {
    ...
    private static final String[] QUERY_ALARMS_WITH_INSTANCES_COLUMNS = {
            ...
            ClockDatabaseHelper.INSTANCES_TABLE_NAME + "." + _ID,
            ...
            ClockDatabaseHelper.INSTANCES_TABLE_NAME + "." + LABEL,
            ClockDatabaseHelper.INSTANCES_TABLE_NAME + "." + VIBRATE
    };
...
//        }
        try {
            Cursor cursor = cr.query(getUri(alarmId), QUERY_COLUMNS, null, null, null);
            if (cursor.moveToFirst()){
                return new Alarm(cursor);
            }
        }catch (Exception e){
...
//        }
        ...
        try{
            Cursor cursor = cr.query(CONTENT_URI, QUERY_COLUMNS, selection, selectionArgs, null);
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    result.add(new Alarm(cursor));
                } while (cursor.moveToNext());
