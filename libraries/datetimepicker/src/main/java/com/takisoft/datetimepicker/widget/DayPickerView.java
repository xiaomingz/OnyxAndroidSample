...
class DayPickerView extends ViewGroup {
    ...
    private void attrHandler(Context context, @Nullable AttributeSet attrs, int defStyleAttr,
                             int defStyleRes) {
        ...
        final int firstDayOfWeek = a.getInt(R.styleable.CalendarView_firstDayOfWeek,
                Calendar.getInstance(Locale.getDefault()).getFirstDayOfWeek()); // LocaleData.get(Locale.getDefault()).firstDayOfWeek
        ...
        final long setDateMillis = Math.max(Math.min(System.currentTimeMillis(), maxDateMillis), minDateMillis);
    ...
    private int getPositionFromDay(long timeInMillis) {
        ...
        return Math.max(Math.min(diffMonth, diffMonthMax), 0);//MathUtils.constrain(diffMonth, 0, diffMonthMax);
    }
