...
public class ViewPagerTabs extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void addTab(CharSequence tabTitle, final int position) {
        ...
        if (mTabIcons != null && position < mTabIcons.length) {
            ...
            if (position == 0) {
                textView.setBackgroundResource(R.drawable.tab_left_sellector);
            } else if (position == mPager.getAdapter().getCount() - 1) {
                textView.setBackgroundResource(R.drawable.tab_right_sellector);
            } else {
                textView.setBackgroundResource(R.drawable.tab_middle_sellector);
            }
