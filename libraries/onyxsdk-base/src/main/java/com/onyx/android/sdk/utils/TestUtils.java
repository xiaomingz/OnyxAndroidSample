...
public class TestUtils {
    ...
    public static Collection<String> generateRandomFiles(String parentPath, Collection<String> files, int randomCount) {
        for (int i = 0; i < randomCount; i++) {
            files.add(TestUtils.generateRandomFile(parentPath, true).getAbsolutePath());
        }
        return files;
    }
    ...
    public static Map<String, Set<String>> generateRandomCollectedFilesMap(final String rootPath) {
        final Map<String, Set<String>> collectedFilesMap = new HashMap<>();
        collectedFilesMap.put(rootPath, new HashSet<String>());
        for (int i = 0; i < randInt(7, 12); i++) {
            String[] keyPaths = collectedFilesMap.keySet().toArray(new String[0]);
            String randomDir = new File(keyPaths[randInt(0, keyPaths.length - 1)], generateUniqueId()).getAbsolutePath();
            FileUtils.mkdirs(randomDir);
            if (!collectedFilesMap.containsKey(randomDir)) {
                collectedFilesMap.put(randomDir, new HashSet<String>());
            }
            generateRandomFiles(randomDir, collectedFilesMap.get(randomDir), 13);
        }
        for (String dir : collectedFilesMap.keySet()) {
            if (CollectionUtils.getSize(collectedFilesMap.get(dir)) < 2) {
                collectedFilesMap.get(dir).add(generateRandomFile(dir, true).getAbsolutePath());
                collectedFilesMap.get(dir).add(generateRandomFile(dir, true).getAbsolutePath());
            }
        }
        return collectedFilesMap;
    }

    public static CountDownLatch createCountDownLatch(int value) {
