...
public class PackageUtils {
    ...
    private static final String LEGACY_PM_INSTALL_COMMAND = "LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install ";
    private static final String PM_INSTALL_COMMAND = "LD_LIBRARY_PATH=/vendor/lib64:/system/lib64 pm install ";
    ...
    private static ComponentName activeInstallerComponent = null;

    private static String getPMInstallCommand() {
        if (CompatibilityUtil.apiLevelCheck(Build.VERSION_CODES.M)) {
            return Process.is64Bit() ? PM_INSTALL_COMMAND : LEGACY_PM_INSTALL_COMMAND;
        }
        return LEGACY_PM_INSTALL_COMMAND;
    }
    ...
    public static int installSilent(Context context, String filePath) {
        return installSilent(context, filePath, " -r " + getInstallLocationParams(getInstallLocation()));
    }

    public static int installSilent(Context context, String filePath, boolean isRoot) {
        return installSilent(context, filePath, " -r " + getInstallLocationParams(APP_INSTALL_INTERNAL), isRoot);
    }
    ...
    public static int installSilent(Context context, String filePath, String pmParams){
        return installSilent(context, filePath, pmParams,!isSystemApplication(context));
    }
    public static int installSilent(Context context, String filePath, String pmParams, boolean isRoot) {
         ...
         **/
        StringBuilder command = new StringBuilder().append(getPMInstallCommand())
                .append(pmParams == null ? "" : pmParams).append(" ").append(filePath.replace(" ", "\\ "));
        CommandResult commandResult = ShellUtils.execCommand(command.toString(), isRoot, true);
    ...
    public static void installSilentByStreamForTargetUser(Context context, Class<? extends BroadcastReceiver> receiverClass, String apkFilePath, int userID) {
        ...
        int sessionId = DPMUtils.createSessionForTargetUser(packageInstaller, sessionParams, userID);
        if (sessionId != -1) {
            boolean copySuccess = copyInstallFile(packageInstaller, sessionId, apkFilePath);
            if (copySuccess) {
                execInstallCommand(context, receiverClass, packageInstaller, sessionId);
            }
        }
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public static void installSilentByStream(Context context, Class<? extends BroadcastReceiver> receiverClass, String apkFilePath) {
        File apkFile = new File(apkFilePath);
        PackageInstaller packageInstaller = context.getPackageManager().getPackageInstaller();
        PackageInstaller.SessionParams sessionParams
                = new PackageInstaller.SessionParams(PackageInstaller
    ...
    private static boolean copyInstallFile(PackageInstaller packageInstaller,
        ...
        try {
            ...
            session.fsync(out);
            Debug.i(PackageUtils.class, "streamed: " + total + " bytes");
            success = true;
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    private static void execInstallCommand(Context context, Class<? extends BroadcastReceiver> receiverClass,
    ...
    public static int getTopTaskId() {
        ActivityManager activityManager = (ActivityManager) ResManager.getAppContext().getSystemService(ACTIVITY_SERVICE);
     ...
     * @return
     */
    private static String getInstallLocationParams(int location) {
//        int location = getInstallLocation();
    ...
    public static File getApkFile(Context context, String packageName) {
        try {
            ...
            return new File(appInfo.publicSourceDir);
        } catch (NameNotFoundException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static int getApkFileSize(Context context, String packageName) {
        File file = getApkFile(context, packageName);
    ...
    public static ComponentName getUsableDefaultInstallerComponentName(Context context) {
        if (activeInstallerComponent != null) {
            return activeInstallerComponent;
        }
        if (ANDROID_DEFAULT_INSTALLER_COMPONENT_NAMES != null) {
            for (ComponentName defaultComponentName : ANDROID_DEFAULT_INSTALLER_COMPONENT_NAMES) {
                if (checkComponentName(context, defaultComponentName)) {
                    activeInstallerComponent = defaultComponentName;
                }
            }
        }
        return activeInstallerComponent;
    ...
    public static String getPermissionControllerName(Context context) {
        ComponentName permissionControllerName = getUsableDefaultInstallerComponentName(context);
        if (permissionControllerName != null) {
            return permissionControllerName.getPackageName();
        }
        return "";
    }
